...
void get_yaw_and_dist(geometry_msgs::Pose current_pose, geometry_msgs::Pose goal_pose,double &dist, double &heading) {
 ROS_INFO("current: x: %f   y: %f",current_pose.position.x,current_pose.position.y);
 ROS_INFO("goal: x: %f   y: %f",goal_pose.position.x,goal_pose.position.y);
 dist = sqrt(pow(goal_pose.position.x-current_pose.position.x,2)+pow(goal_pose.position.y-current_pose.position.y,2));
 if (dist < g_dist_tol) { //too small of a motion, so just set the heading from goal heading
   heading = convertPlanarQuat2Phi(goal_pose.orientation); 
    ...
    heading = atan2(goal_pose.position.y-current_pose.position.y,goal_pose.position.x-current_pose.position.x);
 }
