...
#include <std_msgs/Int32.h> // numerical direction message
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
   ...
   laser_alarm_=0;
   for (int i = ping_index_-100;i<ping_index_;i++){
    ...
    for (int i = ping_index_;i<ping_index_+100;i++){
     if (laser_scan.ranges[i]<MIN_SAFE_DISTANCE) {
         //ROS_WARN("DANGER, WILL ROBINSON!!");
   ...
   std_msgs::Int32 lidar_alarm_msg;
...
int main(int argc, char **argv) {
    ...
    ros::Publisher pub = nh.advertise<std_msgs::Int32>("directional_lidar_alarm", 1);
