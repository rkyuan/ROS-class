...
int g_lidar_alarm=0; // global var for lidar alarm

void alarmCallback(const std_msgs::Int32& alarm_msg) 
{ 
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber alarm_subscriber = n.subscribe("directional_lidar_alarm",1,alarmCallback); 
    //some "magic numbers"
    ...
    while(ros::ok()) { // do forever
      ROS_INFO("start");
        ...
        while(g_lidar_alarm==0) { // keep moving forward until get an alarm signal
          ROS_INFO("forward");
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          ros::spinOnce();
        ...
        while(g_lidar_alarm==1) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          ros::spinOnce();
          loop_timer.sleep();
          }
          //here if got an alarm; turn CW until alarm clears
        twist_cmd.linear.x=0.0; //stop moving forward
        twist_cmd.angular.z=yaw_rate*-1.0; //and start spinning in place
        timer=0.0; //reset the timer
        while(g_lidar_alarm==2) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          ros::spinOnce();
          loop_timer.sleep();
